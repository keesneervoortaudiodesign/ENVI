...
**One item remains open: the 2-D isophone/scenario production map.** `marshalScene.ts` still places receivers on a 1-D corridor (`y=0`), so there is no 2-D field to reconstruct into a meaningful noise map from a real solve; the isophone/difference maps are proven against seeded 2-D grids and are intentionally NOT wired to a degenerate 1-D grid (which would render a false map). Closing it needs engine-validated **2-D CRS-exact scene marshalling** (the plan already exposes 2-D `TierReceiverDto.position`) — deliberately deferred as physics-sensitive.
