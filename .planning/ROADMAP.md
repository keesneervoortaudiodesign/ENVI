...
**Remaining (open): the isophone/scenario 2-D production map only.** `marshalScene.ts` still places receivers on a 1-D corridor (`y=0`), so there is no 2-D field to reconstruct into a meaningful isophone map from a real solve (the isophone map is proven against seeded 2-D grids; the difference/scenario maps likewise). Closing it needs engine-validated **2-D CRS-exact scene marshalling** — the plan already exposes 2-D `TierReceiverDto.position`; wiring it is deliberately deferred as physics-sensitive. See `11-VERIFICATION.md`.
