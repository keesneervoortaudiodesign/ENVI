...
use envi_engine::scene::{Barrier, Building, Source, SubSource};
...
// The scene DTOs the browser solve marshals — terrain / ground / authored
// isolation spectrum / forest params (and their validating engine `TryFrom`s) —
// moved into `envi_compute::scene_dto` (Phase 10, 10-06) so the WASM compute
// boundary can consume them without dragging `std::fs`/`tempfile` into wasm.
// Re-exported here at their original paths so `envi_store::dto::{…}` stay
// source-compatible and keep generating into the committed `wire.ts`.
...
mod tests {
    ...
    // The terrain / ground / isolation / forest DTO conversion + round-trip tests
    // moved with their types into `envi_compute::scene_dto` (10-06); this module
    // keeps the tests for the DTOs that stay in `envi-store`.
