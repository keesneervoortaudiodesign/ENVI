...
//!
//! Factored out of `envi-store` into the WASM-safe `envi-compute` crate (10-06)
//! so the browser solve boundary can interpolate an authored isolation spectrum
//! onto the dense `[105]` grid without dragging `std::fs`/`tempfile` into wasm.
//! `envi-store` re-exports [`interpolate`] (wrapping the error back into its
//! `StoreError`) and [`Resolution`] at their original paths, so its public API
//! and the committed `wire.ts` stay byte-identical.
//!
//! # Module I/O
...
//!   every value must be finite. Out-of-length ⇒ [`SceneDtoError::BadBandCount`];
//!   non-finite ⇒ [`SceneDtoError::NonFinite`].
//!
...
use crate::scene_dto::SceneDtoError;
...
/// [`crate::scene_dto::AuthoredSpectrumDto`] is human-inspectable (D-06).
...
///
/// # Errors
/// - [`SceneDtoError::BadBandCount`] if `values.len()` ≠ the resolution's anchor
///   count (rejected before any allocation over an attacker-controlled length —
///   threat T-07-01-01).
/// - [`SceneDtoError::NonFinite`] if any authored value is NaN or `±∞`.
///
...
pub fn interpolate(
    ...
    values: &[f64],
) -> Result<[f64; N_BANDS], SceneDtoError> {
    let expected = resolution.anchor_count();
    if values.len() != expected {
        return Err(SceneDtoError::BadBandCount { got: values.len() });
    }
    for (i, v) in values.iter().enumerate() {
        if !v.is_finite() {
            return Err(SceneDtoError::NonFinite {
...
mod tests {
    ...
    fn wrong_length_is_bad_band_count() {
        ...
        assert!(matches!(
            ...
            Err(SceneDtoError::BadBandCount { got: 26 })
        ));
    }
