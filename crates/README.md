...
| **`envi-gis`** | The **client-side GIS-ingestion boundary** (DATA-01..04, 08-CONTEXT pivot): a pure-Rust, **sans-I/O** core that decodes cached COG/BigTIFF terrain tiles into windowed `f32` rasters (IFD/geotransform parse, nodata + edge-tile safety); land-cover→impedance and OSM→buildings land in later plans. | `#![deny(unsafe_code)]`; **no network, no OPFS, no browser DOM bindings** — a synchronous API over `&[u8]`, WASM-safe; TypeScript owns all I/O. Reprojection via `envi-geo` only (GEOX-04, no second `proj4rs` edge). Guard-first, no-panic decode: a pre-decode `max_decoded_px` DoS budget from IFD dims (T-08-02-01), typed `GisError` never a panic. | `cog::{decode_window, PixelWindow, Raster, MAX_DECODED_PX}`, `cog::header`, `cog::geo_tags`, `GisError` |
| **`envi-service`** | The single deployable **axum** binary (SVC-03/04): `/api/v1` + the `web/dist` bundle, localhost-bound, refuse-to-start CRS self-check (D-08), job/SSE state machine, recondition/recompute split. Serves the built **`web/`** frontend (Vite + React 19 + MapLibre/Terra Draw scene editor) as a static SPA-fallback bundle. | Thin HTTP layer — routing, jobs, and wire contracts only. Storage delegates to `envi-store`, CRS to `envi-geo`, the DGM TIN to `envi-dgm`, acoustics NEVER here (SVC-07). Long CPU work runs on a dedicated `std::thread`, never tokio's blocking pool (Anti-Pattern 5, D-08). | `cargo run -p envi-service`; `api::app`, `jobs::submit_stub_job`, `selfcheck::crs_self_check` |
